  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 shadow-lg col-span-1 md:col-span-2 lg:col-span-3">
        <h3 className="text-xl font-bold text-foreground mb-4">Survey Data</h3>
  ...
  return (
    <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 shadow-lg col-span-1 md:col-span-2 lg:col-span-3">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-foreground">Survey Data (Editable)</h3>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setCurrentPage(Math.max(0, currentPage - 1))}
            disabled={currentPage === 0}
            className="px-3 py-1 bg-white/20 hover:bg-white/30 disabled:bg-white/10 disabled:cursor-not-allowed text-foreground rounded transition-colors"
          >
            Previous
          </button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/20">
              <th className="text-left p-3 text-foreground/80 font-semibold sticky left-0 bg-white/10">
                #
              </th>
              {headers.slice(0, 10).map((header, index) => (
                <th
                  key={index}
                  className="text-left p-3 text-foreground/80 font-semibold min-w-[150px]"
                >
                  {header}
                </th>
              ))}
          ...
          <tbody>
            {currentData.map((row, rowIndex) => (
              ...
              >
                <td className="p-3 text-foreground/60 font-mono sticky left-0 bg-white/10">
                ...
                {headers.slice(0, 10).map((header, colIndex) => {
                  ...
                  return (
                    <td
                      key={colIndex}
                      className="p-3 text-foreground/90 cursor-pointer hover:bg-white/10 transition-colors"
                    ...
                    >
                      {isEditing ? (
                        <input
                          ...
                          autoFocus
                          className="w-full bg-white/20 text-foreground px-2 py-1 rounded border border-white/30 focus:outline-none focus:border-blue-400"
                        />
                      ) : (
